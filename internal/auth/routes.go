...
func (h *Handler) Mount(r chi.Router, api huma.API, jwtSecret []byte) {
	...
	huma.Register(api, huma.Operation{
		...
		Description: "Public endpoint called by /mel when the user picks a paid plan with card-up-front. Pre-creates a Stripe customer + Checkout session (21-day trial) and returns the hosted Checkout URL. On success, Stripe redirects to the Salvia handoff URL embedded in success_url, which provisions the clinic.",
		Tags:        []string{"Auth"},
		Middlewares: rl,
